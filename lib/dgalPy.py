...
def all(constraintSeq):
    """
    Function extends the functionality of python native all function to return pyomo atomic constraints.

    :param constraintSeq: is a (possibly empty) list of elements, each being either:
                        - bool
                        - pyomo atomic constraint or
                        - sequence of pyomo atomic constraints
    :return: returns either bool (True or False) or a non-empty (flat) sequence of pyomo atomic constraints
    """
    constraint = []
    for c in constraintSeq:
        # TODO update to isinstance
        if type(c) == bool:
            if c is True:
                pass
            elif c is False:
                ...
                raise Exception("dgal.all: bool type error")
        # TODO update to isinstance
...
def min(p):
    optAnswer = optimize(p["model"], p["input"], "min", p["obj"], p["constraints"], p["options"])
    return optAnswer


# def max(model,input,obj,constraints,config):
def max(p):
    optAnswer = optimize(p["model"], p["input"], "max", p["obj"], p["constraints"], p["options"])
    return optAnswer
